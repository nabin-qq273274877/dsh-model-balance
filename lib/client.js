		...
		function BalancePill(props) {
		  ...
		  if (value.kind === "quota") {
		    ...
		    const quotaText = dims !== void 0 ? dims.map(
		      (d) => `${quotaWindowLabel(t, d.window)} ${d.remaining}/${d.limit}`
		    ).join(" \xB7 ") : t("quota.value", { remaining: value.remaining, limit: value.limit });
		    const title = dims !== void 0 ? t("tooltip.quota.dims", {
		      provider: groupName,
		      dims: dims.map(
		        (d) => `${quotaWindowLabel(t, d.window)} ${d.remaining}/${d.limit}` + (d.resetTime !== void 0 ? ` (${t("tooltip.quota.resets")} ${formatReset(d.resetTime)})` : "")
		      ).join(" \xB7 ")
		    }) : t("tooltip.quota", {
		      provider: groupName,
