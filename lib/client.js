window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var POLL_MS = 12e4;
		var TURN_END_SETTLE_MS = 1200;
		function useNoSessionState() {
		  return void 0;
		}
		function selectRunning(snapshot) {
		...
		function BalancePill(props) {
		  ...
		  const useSessionState = props.useSession ?? props.useConversation ?? useNoSessionState;
		  const observedRunning = useSessionState(selectRunning);
		  const running = typeof observedRunning === "boolean" ? observedRunning : props.session?.running === true;
